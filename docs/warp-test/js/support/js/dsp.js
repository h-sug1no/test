...
function setupTypedArray(name, fallback) {
  ...
  const me = window.this || window;
  if (typeof me[name] !== 'function' && typeof me[name] !== 'object') {
    // nope.. check if WebGLArray exists
    if (
      typeof me[fallback] === 'function' &&
      typeof me[fallback] !== 'object'
    ) {
      me[name] = me[fallback];
    } else {
      // nope.. set as Native JS array
      me[name] = function (obj) {
...
RFFT.prototype.forward = function (buffer) {
  ...
  for (let k = 0, len = reverseTable.length; k < len; k++) {
...
function Sampler(
  ...
  this.loadSamples = function (event) {
    let buffer = DSP.getChannel(DSP.MIX, event.frameBuffer);
...
Oscillator.prototype.generate = function () {
  ...
  for (let i = 0; i < this.bufferSize; i++) {
    //let step = (frameOffset + i) * this.cyclesPerSample % 1;
...
function Biquad(type, sampleRate) {
  ...
  this.coefficients = function () {
    let b = [this.b0, this.b1, this.b2];
    let a = [this.a0, this.a1, this.a2];
    return { b: b, a: a };
  };
  ...
  this.recalculateCoefficients = function () {
    ...
    let w0 = (DSP.TWO_PI * this.f0) / this.Fs;
  ...
  this.process = function (buffer) {
    ...
    let output = new Float64Array(len);
...
DSP.freqz = function (b, a, w) {
  ...
  let result = Float64Array(w.length);
...
function GraphicalEq(sampleRate) {
  ...
  this.recalculateFilters = function () {
    let bandCount = Math.round(
      (Math.log(this.maxFreq / this.minFreq) * this.bandsPerOctave) / Math.LN2,
    );

    this.filters = [];
    for (let i = 0; i < bandCount; i++) {
      let freq = this.minFreq * Math.pow(2, i / this.bandsPerOctave);
      let newFilter = new Biquad(DSP.PEAKING_EQ, this.FS);
...
MultiDelay.prototype.process = function (samples) {
  ...
  let outputSamples = new Float64Array(samples.length);
  ...
  for (let i = 0; i < samples.length; i++) {
    ...
    let sample = delaySample * this.delayVolume + samples[i];
...
SingleDelay.prototype.process = function (samples) {
  ...
  for (let i = 0; i < samples.length; i++) {
    ...
    // delayBufferSamples could contain initial NULL's, return silence in that case
    let delaySample = this.delayBufferSamples[this.delayOutputPointer];
...
Reverb.prototype.process = function (interleavedSamples) {
  // NB. Make a copy to put in the output samples to return.
  let outputSamples = new Float64Array(interleavedSamples.length);

  // Perform low pass on the input samples to mimick damp
  let leftRightMix = DSP.deinterleave(interleavedSamples);
  this.LOWPASSL.process(leftRightMix[DSP.LEFT]);
  this.LOWPASSR.process(leftRightMix[DSP.RIGHT]);
  let filteredSamples = DSP.interleave(
  ...
  let singleDelaySamples = new Float64Array(outputSamples.length);
  for (i = 0; i < this.NR_OF_SINGLEDELAYS; i++) {
